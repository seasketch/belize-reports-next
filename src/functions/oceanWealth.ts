...
/**
 * oceanWealth: A geoprocessing function that calculates overlap metrics for vector datasources
 * @param sketch - A sketch or collection of sketches
 * @param extraParams
 * @returns Calculated metrics and a null sketch
 */
export async function oceanWealth(
  sketch:
  ...
  const metricGroup = project.getMetricGroup("oceanWealth");
  const metrics = (
    await Promise.all(
...
export default new GeoprocessingHandler(oceanWealth, {
  title: "oceanWealth",
  description: "",
  timeout: 500, // seconds
